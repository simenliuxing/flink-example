...
public class DataGenExample {
    public static void main(String[] args) throws Exception {
        ...
        // 解析source端数据
        SingleOutputStreamOperator<Person> map = env.addSource(new SourceFunction<String>() {
            @Override
            public void run(SourceContext<String> sourceContext) throws InterruptedException {
                while (true) {
                    sourceContext.collect("{\"name\":\"dtstack\",\"age\":\"12\",\"timestamp\":\"" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")) + "\"}");
                    Thread.sleep(1000);
                }
            }
